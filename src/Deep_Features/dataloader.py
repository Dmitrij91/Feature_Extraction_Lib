...
class OCTPatchDataset(data.Dataset):
    ...
    def __init__(self, data_path, labels_path, size_window, c, single_voxel_out=True):
        """
        """
        self.raw_data = np.load(data_path).astype(np.float32)
        assert self.raw_data.ndim == len(size_window)
        if self.raw_data.shape[0] < size_window[0]:
            self.view_spatial_shape = (0,0,0)
            return
        self.size_window = size_window
        self.size_outer_window = (size_window[0], size_window[1], size_window[2])
        try:
        ...
        assert c_dat in [12, 14], f"Unexpected number of labels {c_dat} in {labels_path}"
        ...
        if single_voxel_out:
            rx, ry, rz = (self.size_outer_window[0] - 1)//2, (self.size_outer_window[1] - 1)//2, (self.size_outer_window[2] - 1)//2
            if rx == 0:
                self.labels = self.labels[:,ry:-ry,rz:-rz]
            else:
                self.labels = self.labels[rx:-rx,ry:-ry,rz:-rz]
            assert self.labels.shape == self.view_spatial_shape
        else:
            self.label_view = view_as_windows(self.labels, self.size_outer_window)
