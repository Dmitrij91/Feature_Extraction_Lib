"""
This model architecture code was taken from https://towardsdatascience.com/residual-network-implementing-resnet-a7da63c7b278
and adapted for 3d input data.
"""
...
class ResNetEncoder(nn.Module):
    ...
    def __init__(self, in_channels=1, blocks_sizes=[64, 128, 256, 512], depths=[2,2,2,2], 
                 activation='relu', block=ResNetBasicBlock, kernel_size_enc=3, *args, **kwargs):
        super().__init__()
        self.blocks_sizes = blocks_sizes

        assert kernel_size_enc in [3, 7]
        padding_enc = 1 if kernel_size_enc == 3 else 3
        
        self.gate = nn.Sequential(
            nn.Conv3d(in_channels, self.blocks_sizes[0], kernel_size=kernel_size_enc, stride=2, padding=padding_enc, bias=False),
...
class ResnetDecoder(nn.Module):
    ...
    """
    def __init__(self, in_features, n_classes, dropout=0.0):
        super().__init__()
...
class ResNet(nn.Module):
    ...
    def __init__(self, in_channels, n_classes, dropout=0.0, *args, **kwargs):
        super().__init__()
        self.encoder = ResNetEncoder(in_channels, *args, **kwargs)
        self.decoder = ResnetDecoder(self.encoder.blocks[-1].blocks[-1].expanded_channels, n_classes, dropout)
...
def resnet(size, in_channels, n_classes, *args, **kwargs):
    layer_depths = {
        5:   [1, 1, 1, 1],
        6:   [1, 1, 1, 1],
        7:   [1, 1, 1, 1],
    ...
    return ResNet(in_channels, n_classes, block=blocks[size], blocks_sizes=channel_dims[size], depths=layer_depths[size], *args, **kwargs)